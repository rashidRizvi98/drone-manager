...
export const validateToLoadMedication = async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        ...
        if (!medication) {
            return res.status(400).json({ error: 'Invalid medication code' });
        }
    
        if (drone?.batteryPercentage! < 25) {
            return res.status(400).json({ error: 'The battery level is lower than 25%, please recharge' });
        }
    
        if (drone?.state == DroneStateEnum.LOADING) {
            return res.status(400).json({ error: 'Please wait until current loading completes' });
        }
